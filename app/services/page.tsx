...
export default function Services() {
  return (
    <main className="h-screen w-full bg-black text-white antialiased flex flex-col relative overflow-hidden">
      ...
      <div className="flex-grow w-full max-w-7xl mx-auto flex flex-col justify-center items-center z-20 px-8 py-10 gap-y-10">
        
        <div className="text-center">
          <h4 className="text-[10px] uppercase tracking-[0.4em] text-zinc-500 mb-4 font-light">Services</h4>
          <p className={`${montserrat.className} text-zinc-100 font-light text-2xl tracking-wide max-w-2xl mx-auto leading-relaxed`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
          {[ 
            ...
            <div key={i} className="flex flex-col items-center text-center p-12 border border-white/10 bg-black/50 backdrop-blur-md rounded-2xl transition-all duration-500 hover:bg-black/70 hover:border-white/20 hover:scale-[1.02]">
              <h3 className={`${montserrat.className} text-base font-medium mb-4 tracking-widest uppercase text-white`}>{service.title}</h3>
              <p className="text-[12px] text-zinc-300 font-light leading-relaxed max-w-[220px]">{service.desc}</p>
            </div>
          ))}
        </div>
      </div>
